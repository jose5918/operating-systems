...
void Scheduler() {               // choose a PID as current_pid to load/run
  ...
  if (current_pid > 9){
    ch_p[current_pid*80+40] = 0xf00 + 1 + '0';
    ch_p[current_pid*80+41] = 0xf00 + (current_pid - 10) + '0';
  } else{
    ch_p[current_pid*80+41] = 0xf00 + current_pid + '0';
  }
