#!/usr/bin/perl
# build.pl
# Author: W. Chang, copyfree 2017
# Perl script to compile, link, and convert executable to text file
#    input: .s file, e.g., "./build.pl sleep3.s"
#    output: .txt, e.g., "sleep3.txt"

$#ARGV == 0 || die "Usage: ./build.pl xxx.s (e.g., ./build.pl sleep3.s)\n";

$ARGV[0] =~ s/\.s//;                        # get rid of ".s" part of input filename
`as --32 $ARGV[0].s -o $ARGV[0].o`;         # GNU assembloer outputs ".o" file
`link386 -M -b 16m -nostartfiles $ARGV[0].o -o $ARGV[0]`; # linked at 0x01000080

$in_file = $ARGV[0];                         # compile-linked binary filename
$out_file = "$in_file.txt";                  # convert binary file to ".txt" file
...
for $i (0 .. $in_size - 1) {
   next if($i < 128); # skip first 128 bytes - header (not writing it out)
